...
var camelCase__default = /*#__PURE__*/_interopDefault(camelCase$1);
...
function createContext(options) {
  ...
  function useContext(fallback) {
    ...
    var context = vue.inject(contextSymbol, fallback);
...
function isObjectComponent(subject) {
  ...
  if (utils.isObject(subject)) {
    // Is object component with render function
    if (typeof (subject == null ? void 0 : subject.render) === "function" && vue.isVNode(subject.render())) return true; // Is object component with setup function
...
function useDebouncedRef(initialValue, delay, immediate) {
  ...
  var debouncedRef = vue.customRef(function (track, trigger) {
    return {
      get: function get() {
        track();
        return state.value;
...
var defaultWindow = utils.canUseDOM() ? window : null;
...
function focusIn(container, focus) {
  var elements = Array.isArray(container) ? container.slice().sort(function (a, z) {
    ...
    var focusables = utils.getAllFocusable(container).filter(function (el) {
      return el !== container;
    });
    return focusables;
  }();
  ...
  var direction = function () {
    if (focus & (exports.Focus.First | exports.Focus.Next)) return Direction.Next;
    if (focus & (exports.Focus.Previous | exports.Focus.Last)) return Direction.Previous;
    throw new Error("Missing Focus.First, Focus.Previous, Focus.Next or Focus.Last");
  }();

  var startIndex = function () {
    if (focus & exports.Focus.First) return 0;
    if (focus & exports.Focus.Previous) return Math.max(0, elements.indexOf(active)) - 1;
    if (focus & exports.Focus.Next) return Math.max(0, elements.indexOf(active)) + 1;
    if (focus & exports.Focus.Last) return elements.length - 1;
    throw new Error("Missing Focus.First, Focus.Previous, Focus.Next or Focus.Last");
  }();

  var focusOptions = focus & exports.Focus.NoScroll ? {
    preventScroll: true
  } : {};
  var offset = 0;
  ...
  do {
    ...
    if (offset >= total || offset + total <= 0) return exports.FocusResult.Error;
    var nextIdx = startIndex + offset;

    if (focus & exports.Focus.WrapAround) {
      nextIdx = (nextIdx + total) % total;
    } else {
      if (nextIdx < 0) return exports.FocusResult.Underflow;
      if (nextIdx >= total) return exports.FocusResult.Overflow;
    }
  ...
  return exports.FocusResult.Success;
}

var camelCaseCache = {};

var _isStyledProp = memoize__default["default"](function (attr) {
  return styledSystem.isStyleProp(attr);
});
...
var camelCase = memoize__default["default"](function (key) {
  return camelCase__default["default"](key);
});

Object.defineProperty(exports, 'canUseDOM', {
  enumerable: true,
  get: function () { return utils.canUseDOM; }
});
exports.SAO = SAO;
exports.SNA = SNA;
exports.SNAO = SNAO;
...
exports.isObjectComponent = isObjectComponent;
exports.match = match;
...
exports.vueThemingProps = vueThemingProps;

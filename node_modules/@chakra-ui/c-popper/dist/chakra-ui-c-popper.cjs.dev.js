...
var defaultModifiers = [core.eventListeners, core.popperOffsets, core.computeStyles, core.applyStyles];
function createPopperFn(options) {
  var _options$offset;

  return core.popperGenerator({
    defaultOptions: {
      placement: "bottom",
      strategy: "absolute",
      modifiers: []
    },
    defaultModifiers: [].concat(defaultModifiers, [positionArrow, innerArrow, transformOrigin, _extends({}, core.eventListeners, getEventListenerOptions(options.eventListeners)), _extends({}, core.arrow, {
      options: {
        padding: options.arrowPadding
      }
    }), _extends({}, core.offset, {
      options: {
...
function usePopper(props) {
  ...
  var options = _extends({}, defaultProps, utils.filterUndefined(props));

  var _options$modifiers = options.modifiers,
      modifiers = _options$modifiers === void 0 ? [] : _options$modifiers,
      placement = options.placement,
      strategy = options.strategy;
  var createPopper = vue.ref(createPopperFn(options));

  var _reference = vue.ref(null);

  var _popper = vue.ref(null);

  var popperInstance = vue.ref(null); // cleanup previous instance handler

  var cleanup = vue.ref(function () {});
  var unsubscribe;
  ...
  var setup = function setup() {
    vue.nextTick().then(function () {
      ...
      cleanup.value == null ? void 0 : cleanup.value(); // @ts-expect-error Rewrite this hook to use native popper
      ...
      popperInstance.value.forceUpdate(); // @ts-expect-error Rewrite this hook to use native popper
  ...
  vue.onMounted(function () {
    vue.nextTick().then(function () {
      unsubscribe = vue.watch(function () {
...
exports.popperCSSVars = popperCSSVars;
